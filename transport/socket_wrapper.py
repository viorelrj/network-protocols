...
class Connection:
    ...
    def connect_client(self, addr):
        ...
        if self.__syn == 0:
            ...
            while not (response or ('ack' in response and response['ack'] != self.__syn)) and tries < self.__max_tries:
                self.__core.sendto(packet.encode_packet(syn=self.__syn), self.__addr)
                tries += 1
            ...
            self.__core.sendto(packet.encode_packet(ack=self.__ack, syn=self.__syn), addr)
    ...
    def close_server(self, data: None):
        if (data != None):
            self.__ack += 1
            pack = packet.encode_packet(fin=True, ack=self.__ack, syn=self.__syn)
            self.__syn += 1
    ...
    def send(self, message):
        ...
        encoded = packet.encode_packet(ack=self.__ack, syn=self.__syn, data=message, data_len=data_len)
        self.__syn += data_len
